...
class Likelihood_eft(Likelihood):
    ...
    def __init__(self, path, data, command_line):
        ...
        # read values of k (in h/Mpc)
        kPS, PSdata = self.__load_data()
        self.k = kPS[:len(kPS) // 3]
        self.ps = PSdata.reshape((3, len(PSdata) // 3))

        indexkred = np.argwhere((kPS <= self.kmax) & (kPS >= self.kmin))[:, 0]
        xdata = kPS[indexkred]
        ydata = PSdata[indexkred]
        N = 2 * len(ydata) // 3
        self.xdata = xdata[:int(N/2)]
        self.ydata = ydata[:N]
        self.Nk = int(N/2)
        ...
        if self.use_covmat:
            cov = np.loadtxt(os.path.join(self.data_directory, self.covmat_file)) #/ 4.25
            covred = cov[indexkred.reshape((len(indexkred), 1)), indexkred][:N, :N]
        ...
        try: 
            if self.omega_b_BBNsigma is not 0: 
                ...
                print ('setting BBN prior on omega_b')
            else: print ('setting no prior on omega_b')
        except:
            print ('no BBN prior specified: setting no prior on omega_b')

        self.use_fbcprior = False
        try: 
            if self.fbc_sigma is not 0: 
                self.use_fbcprior = True
                print ('setting prior on fbc')
            #else: print ('setting no prior on fbc')
        except: pass
            #print ('no BBN prior specified: setting no prior on omega_b')
    ...
    def get_Covbi_for_marg(self, Pi_data, Cinv, sigma=4.):
...
class Likelihood_bird(Likelihood_eft):
    ...
    def __init__(self, path, data, command_line):
        ...
        try:
            self.Qll = np.load(os.path.join(self.data_directory, self.Qll_file))
            self.setk_or = np.loadtxt(os.path.join(self.data_directory, self.setk_or_file))
            self.setkp_or = np.loadtxt(os.path.join(self.data_directory, self.setkp_or_file))
            self.use_window = True
            print("Window function: on")
        except:
            print("Window function: none")
            self.Qll = None
            self.setk_or = None
            self.setkp_or = None
        ...
        self.projection = pb.Projection(self.xdata, self.Om_AP, self.zAP, nbinsmu=200, Qll=self.Qll, k_or=self.setk_or, kp_or=self.setkp_or, co=self.co)
    ...
    def loglkl(self, cosmo, data):
        ...
        if self.birdlkl is 'fastmarg' or self.birdlkl is 'fastfull':
            if data.need_cosmo_update is True or self.bird is None: 
                ...
                self.bird = pb.Bird(self.kin, plin, cosmo.Omega_m(), self.z, which='all', co=self.co)
            ...
            if self.birdlkl is 'fastfull': ### NOT TESTED ; 
            #   bug in the following because kout != kbird 
        ...
        if self.use_fbcprior:
            prior += -0.5 * ((data.cosmo_arguments['omega_b'] - self.fbc_center * data.cosmo_arguments['omega_cdm']) / self.fbc_sigma)**2
    ...
    def __get_Pi_for_marg(self, Pct, Pb3, b1, f, model=2):
        Pi = np.array([ 
                        0.5* Pb3,                                         # *b3
                        0.5* (2*f*Pct[:,0+3]+2*b1*Pct[:,0]) / self.knl**2, # *b5
                        1.0* (2*f*Pct[:,1+3]+2*b1*Pct[:,1]) / self.km**2 , # *b6 
                        1.0* (2*f*Pct[:,2+3]+2*b1*Pct[:,2]) / self.km**2 , # *b7
                    ])

        if model == 1: 
            Onel0 = np.array([np.ones(self.Nk), np.zeros(self.Nk)]) # shot-noise mono
            kl2 = np.array([np.zeros(self.Nk), self.xdata]) # k^2 quad
            Pi = np.concatenate(( Pi, 
                np.array([
                    100. * Onel0,
                    0.5*kl2**2 / self.nd / self.km**2
                        ]) ))
...
class Likelihood_taylor(Likelihood_eft):
    ...
    def __get_Pi_for_marg(self, Ploop, kfull, b1, model=2, withBisp=False):
        ...
        if model == 1:
            ...
            Pi = np.array([0.5 * Ploop[:, 3, :] + b1 * Ploop[:, 7, :],
                           0.5 * (Ploop[:, 15, :] + b1 * Ploop[:, 12, :]) / self.knl**2,
                           1. * (Ploop[:, 16, :] + b1 * Ploop[:, 13, :]) / self.km**2,
                           1. * (Ploop[:, 17, :] + b1 * Ploop[:, 14, :]) / self.km**2,  # b7
                           100. * Onel0,
                           0.5 * kl2**2 / self.nd / self.km**2])  # this is k2 quad
        elif model == 2:
            kl2 = np.array([np.zeros(nk), kfull])
            Pi = np.array([0.5 * (Ploop[:, 3, :] + b1 * Ploop[:, 7, :]), # b3
                           0.5 * (Ploop[:, 15, :] + b1 * Ploop[:, 12, :]) / self.knl**2, # b5
                           1. * (Ploop[:, 16, :] + b1 * Ploop[:, 13, :]) / self.km**2,  # b6
                           1. * (Ploop[:, 17, :] + b1 * Ploop[:, 14, :]) / self.km**2,  # b7
                           0.5 * kl2**2 / self.nd / self.km**2])
