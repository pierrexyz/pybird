...
class ReadWrite(object):
    ...
    def check(self, c, d, verbose=True):
        ...
        options_in_config = ['with_bao_rec', "with_ap", "with_survey_mask", "with_binning", "with_wedge", "with_redshift_bin"]
    ...
    def config(self, c, fd_sky):
        ...
        for sky, fd in zip(c['sky'].keys(), fd_sky):
            ...
            if c['with_binning']: fc['binsize'] = fd['binsize']
    ...
    def write(self, c, fd_sky, out=None):
        ...
        for fdata, o, sky in zip(fd_sky, out, c['sky']):
            ...
            if c['write']['plot']: 
                ...
                plt.xlabel(xlabel)
                plt.ylabel(ylabel)
                if c['write']['save']: plt.savefig(os.path.join(c['write']['out_path'], 'plot_%s_%s.pdf') % (c['write']['out_name'], sky), bbox_inches='tight')
