...
class Projection(object):
    ...
    The Projection class handles various observational effects that transform the theoretical
    prediction into a form that can be directly compared with survey data. These effects include:
    
    - Alcock-Paczynski (AP) effect: Accounts for the distortion due to assuming an incorrect
      fiducial cosmology when converting redshifts to distances.
    ...
    - Fiber collision corrections: Applies corrections for missing galaxy pairs due to
      fiber collisions in spectroscopic surveys.
    ...
    Attributes:
        co (Common): Common parameters object with configuration settings.
        cf (bool): Whether working with correlation function (True) or power spectrum (False).
        xout (ndarray): Output k or s array matching the data.
        
        # AP effect attributes
        H_fid (float): Fiducial Hubble parameter.
        D_fid (float): Fiducial angular diameter distance.
        muacc (ndarray): Mu grid for AP integration.
        sgrid, musgrid (ndarray): Meshgrid of s and mu for correlation function.
        kgrid, mukgrid (ndarray): Meshgrid of k and mu for power spectrum.
        arrayLegendremusgrid (ndarray): Legendre polynomials evaluated on musgrid.
        arrayLegendremukgrid (ndarray): Legendre polynomials evaluated on mukgrid.
        
        # Survey window attributes
        arr_p (ndarray): Array of p values for window function.
        mat_kp (ndarray): Window function matrix.
        
        # Binning attributes
        binvol (ndarray): Volume of each k-bin.
        points (list): List of k-values within each bin.
        
        # Wedge attributes
        wedge_mat_wl (ndarray): Transformation matrix from multipoles to wedges.
        
        # Redshift bin attributes
        zz (ndarray): Array of redshift values across the bin.
        nz (ndarray): Number density as function of redshift.
        s, z1, mu (ndarray): Meshgrid of separation, redshift and angle.
        n1 (ndarray): Number density evaluated on the meshgrid.
        L (ndarray): Legendre polynomials for 3D reconstruction.
        Lp (ndarray): Legendre polynomials for multipole integration.
        ft (FourierTransform): Fourier transform object for s-k conversions.
    
    Methods:
        get_AP_param(): Compute the AP parameters qperp and qpar.
        integrAP(): Perform AP integration over mu.
        AP(): Apply the AP effect to power spectrum or correlation function.
        
        integrWindow(): Convolve window function with power spectrum.
        Window(): Apply survey window function to power spectrum.
        
        dPuncorr(): Compute uncorrelated fiber collision contribution.
        dPcorr(): Compute correlated fiber collision contribution.
        fibcolWindow(): Apply fiber collision corrections.
        
        loadBinning(): Create bins for data k-array.
        integrBinning(): Integrate over each k-bin.
        xbinning(): Apply k-space binning for linear-spaced data.
        xdata(): Interpolate to data k-array.
        
        integrWedges(): Convert multipoles to wedges.
        Wedges(): Apply multipole to wedge transformation.
        Wedges_external(): Apply wedge transformation to external data.
        
        mesheval1d(): Evaluate 1D function on mesh grid.
    ...
    def loadBinning(self, xout, binsize):
        ...
        self.points = [linspace(kbinmin, kbinmax, 30) for (kbinmin, kbinmax) in zip(binmin, binmax)]
