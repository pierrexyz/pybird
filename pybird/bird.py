...
class Bird(object):
            ...
            if self.with_bias: # evaluation with biases specified
                for i in range(self.co.Nl):
                    ...
                    if self.co.exact_time:
                        ...
                        if self.co.with_uvmatch or self.co.with_irmatch: self.b13[i] = array([ b1**2*G1**2*mu[0][l], b1*b3*mu[0][l], b1*f*G1**2*mu[2][l], b3*f*mu[2][l], f**2*G1**2*mu[4][l], b1**2*Y1*mu[0][l], b1*f*mu[2][l]*Y1, f**2*mu[4][l]*Y1, b1**2*f*G1t*mu[2][l], b1*f**2*G1t*mu[2][l], b1*f**2*G1t*mu[4][l], f**3*G1t*mu[4][l], f**3*G1t*mu[6][l], b1*f*mu[2][l]*V12t, f**2*mu[4][l]*V12t,
                        ...
                        if self.co.with_uvmatch or self.co.with_irmatch: self.b13[i] = array([ b1**2 * mu[0][l], b1 * b3 * mu[0][l], b1**2 * f * mu[2][l], b1 * f * mu[2][l], b3 * f * mu[2][l], b1 * f**2 * mu[2][l], b1 * f**2 * mu[4][l], f**2 * mu[4][l], f**3 * mu[4][l], f**3 * mu[6][l],
                ...
                if self.co.exact_time:      # config["with_exact_time"] == True
                    if self.co.Nloop == 12: # config["with_time"] == True
                        ...
                        if self.co.with_uvmatch or self.co.with_irmatch:
                            ...
                            self.Cloopl = self.Ploopl.at[:, 0].add(f1**4 * self.C13l[:, -1] + f1**3 * self.C13l[:, -2]) # *1
                            self.Cloopl = self.Ploopl.at[:, 1].add(f1**3 * self.C13l[:, -3] + f1**2 * self.C13l[:, -4]) # *b1
                            self.Cloopl = self.Ploopl.at[:, 5].add(f1**2 * self.C13l[:, -5] + f1 * self.C13l[:, -6]) # *b1*b1
    ...
    def setreducePslb(self, bs, what="full"):
        ...
        if "full" in what:
            ...
            if self.with_stoch: self.Ps[1] += einsum('b,lbx->lx', self.bst, self.Pstl)
            if self.with_nnlo_counterterm: self.Ps[2] = einsum('b,lbx->lx', self.cnnlo, self.Pnnlol)
        else:
            if "linear" in what: self.Ps[0] = einsum('b,lbx->lx', self.b11, self.P11l)
            if "sptloop" in what:
                if "sptloop22" in what and self.co.keep_loop_pieces_independent: self.Ps[1] = einsum('b,lbx->lx', self.bloop[:self.co.N22], self.Ploopl[:, :self.co.N22])
                elif "sptloop13" in what and self.co.keep_loop_pieces_independent: self.Ps[1] = einsum('b,lbx->lx', self.bloop[self.co.N22:], self.Ploopl[:, self.co.N22:])
                else: self.Ps[1] = einsum('b,lbx->lx', self.bloop, self.Ploopl)
            if "spt" not in what or "counterterm" in what: self.Ps[1] += einsum('b,lbx->lx', self.bct, self.Pctl)
            if "stochastic" in what and self.with_stoch: self.Ps[1] += einsum('b,lbx->lx', self.bst, self.Pstl)
            if "nnlo_counterterm" in what and self.with_nnlo_counterterm: self.Ps[2] = einsum('b,lbx->lx', self.cnnlo, self.Pnnlol)
            if "ir_correction" in what:
                self.Ps[0] = einsum('b,lbx->lx', self.b11, self.fullIRPs11)
                self.Ps[1] = einsum('b,lbx->lx', self.bloop, self.fullIRPsloop) + einsum('b,lbx->lx', self.bct, self.fullIRPsct)
        if self.Ps[2] is None:
            self.Ps[2] = zeros_like(self.Ps[0])
    ...
    def settime(self, cosmo, co=None):
        ...
        if self.co.nonequaltime:
            ...
            Dp2n = concatenate(( 2*[self.co.Na*[Dp2**(n+1)] for n in range(self.co.NIR)] ))
