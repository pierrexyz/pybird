...
class NonLinear(object):
    ...
    def __init__(self, load_matrix=True, save_matrix=True, path='./', NFFT=256, fftbias=-1.6, co=co):
    ...
    def setMcf22(self):
        ...
        self.Mcf22 = einsum('lnm,bnm->blnm', self.Ml, self.M22)
    ...
    def setMcf13(self):
        ...
        self.Mcf13 = einsum('lnm,bn->blnm', self.Ml, self.M13)
    ...
    def setkPow(self):
        ...
        self.kPow = exp(einsum('n,k->nk', self.fft.Pow, log(self.co.k)))
    ...
    def CoefkPow(self, Coef):
        ...
        return einsum('n,nk->nk', Coef, self.kPow)
    ...
    def makeP13(self, CoefkPow, bird):
        ...
        bird.P13 = self.co.k**3 * bird.P11 * real(einsum('nk,bn->bk', CoefkPow, self.M13))
    ...
    def Coef(self, bird, window=None):
        ...
        """
        if is_jax:
            coef_jit = jit(self.fft.Coef)
            return coef_jit(bird.kin, bird.Pin_2, window=window)
        else: return self.fft.Coef(bird.kin, bird.Pin_2, window=window)
...
def M13a(n1):
    ...
    return tan(n1 * pi) / (14. * (-3 + n1) * (-2 + n1) * (-1 + n1) * n1 * pi)
