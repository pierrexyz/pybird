...
class Likelihood(object):
    ...
    def set_class_settings(self):
        ...
        self.kin = logspace(-5, log10kmax, 200)
        return
    ...
    def set_eft_parameters(self):
        ...
        self.bg_prior_mean = array([self.c["eft_prior"][param]["mean"] for param in self.bg_name])
        self.bg_prior_sigma = array([self.c["eft_prior"][param]["range"] for param in self.bg_name]) 
        self.bng_name = [param for param, prior in self.c["eft_prior"].items() if prior["type"] == 'gauss']
        self.bng_prior_mean = array([self.c["eft_prior"][param]["mean"] for param in self.bng_name])
        self.bng_prior_sigma = array([self.c["eft_prior"][param]["range"] for param in self.bng_name])
        self.bnlog_name = [param for param, prior in self.c["eft_prior"].items() if prior["type"] == 'lognormal']
        self.bnlog_prior_mean = array([self.c["eft_prior"][param]["mean"] for param in self.bnlog_name])
        self.bnlog_prior_sigma = array([self.c["eft_prior"][param]["range"] for param in self.bnlog_name])
    ...
    def get_chi2_marg(self, Tng_k, Tg_bk, P): 
        ...
        chi2 = F0 - einsum('a,b,ab->', F1, F1, invF2, optimize=self.optipath_chi2) + linalg.slogdet(F2)[1] 
    ...
    def get_chi2_non_marg(self, T_k, P):
        ...
        chi2 = einsum('k,p,kp->', T_k, T_k, P, optimize=self.optipath_chi2) # 
        return chi2
    ...
    def get_prior(self, b_sky):
        ...
        def _get_prior(bs, prior_mean, prior_sigma, prior_inv_corr_mat=None, prior_type='gauss'):
            if prior_type == 'gauss': 
                ...
                else: prior = - 0.5 * einsum( 'n,nm,m->', log(bs) - prior_mean, prior_sigma**-2 * prior_inv_corr_mat, log(bs) - prior_mean ) #- sum( log(einsum('n,nm->m', bs, linalg.inv(prior_inv_corr_mat))), axis=0 ) #- sum(log(bs), axis=0)          
        ...
        if self.marg_lkl and self.c["get_maxlkl"]: 
            for i, param in enumerate(self.bg_name): prior += _get_prior(array([b_sky[j][param] for j in range(self.nsky)]), self.bg_prior_mean[i], self.bg_prior_sigma[i], self.prior_inv_corr_matrix, prior_type='gauss')
        return prior
    ...
    def loglkl(self, free_b, free_b_name, class_engine, need_cosmo_update=True): 
        ...
        free_b_name_sky = array(free_b_name[pad:]).reshape(self.nsky, -1)
        ...
        if self.marg_lkl:
            ...
            chi2, bg = self.get_chi2_marg(concatenate(Tng_k)-self.y_all, block_diag(*Tg_bk), self.p_all) 
